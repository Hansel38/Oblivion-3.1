...
// ===== PRIORITY 4.4: Memory Integrity =====
static MemoryIntegrity* g_pMemoryIntegrity = nullptr;
// ===== PRIORITY 4.5: Anti-Tampering =====
...
namespace {
 ...
 GetProcessNameByPid(offenderPid, offenderExe);
            ...
            outReason = std::wstring(bufReason);
 return true;
 }
 }
 }
...
static void SendDetectionJson(const DetectionResult& result, const char* subtype)
{
    if (g_pNetClient != nullptr) {
        ...
        // Log detection being sent
    LOG_WARNING_FMT("Detection sent - Type: %s, Process: %s (PID: %u), Reason: %s", 
                       subtype ? subtype : "unknown",
                       WToUtf8(result.processName).c_str(),
                       result.pid,
                       WToUtf8(result.reason).c_str());
    } else {
        LOG_ERROR("Cannot send detection - NetworkClient is NULL");
    }
}
...
{
    ObfuscatedAPI::ObfEnumWindows(EnumCloseWindowsProc, static_cast<LPARAM>(pid));
...
static bool CheckCheatEngineArtifactsInProcess(DWORD pid, std::wstring& outProcName, std::wstring& outReason)
{
    ...
    std::vector<std::wstring> tokens;
    {
        ...
        if (tokens.empty()) {
            // fallback to defaults if somehow empty (use obfuscated literals)
            tokens = { 
                OBFUSCATE_W("vehdebug"), 
                OBFUSCATE_W("speedhack"), 
                OBFUSCATE_W("dbk"), 
                OBFUSCATE_W("cheatengine"), 
                OBFUSCATE_W("ceserver"), 
                OBFUSCATE_W("celua"), 
                OBFUSCATE_W("monohelper") 
            };
        }
    }
...
static bool DetectCheatEngineDriver(std::wstring& outName)
{
    ...
    for (int i = 0; i < count; ++i) {
        if (GetDeviceDriverBaseNameW(drivers[i], name, MAX_PATH)) {
            ...
            if (base.find(dbkTok) != std::wstring::npos || base.find(ceDrvTok) != std::wstring::npos) {
...
static void SchedulePeriodicScans()
{
 ...
 if (g_pMemoryIntegrity && g_pScanPrioritizer) {
     g_pScanPrioritizer->ScheduleTask("MemoryIntegrityMonitor", []() -> bool {
         ...
         // Check for memory modifications (hash mismatches)
         if (g_pMemoryIntegrity->DetectMemoryModifications()) {
             ...
             for (const auto& mod : modifications) {
                 ...
                 wchar_t addrbuf[32]; swprintf_s(addrbuf, L"0x%p", mod.address);
                 dr.processName = std::wstring(mod.regionName.begin(), mod.regionName.end());
                 
                 std::wstring modType(mod.modificationType.begin(), mod.modificationType.end());
                 if (mod.modificationType == "HASH_MISMATCH_CRC32") {
                     dr.reason = std::wstring(L"Memory integrity violation (CRC32): region='") + dr.processName + 
                                L"', addr=" + addrbuf + 
                                L", expected=0x" + std::to_wstring(mod.expectedHash) + 
                                L", actual=0x" + std::to_wstring(mod.actualHash);
                 } else {
                     dr.reason = std::wstring(L"Memory integrity violation (") + modType + L"): region='" + 
                                dr.processName + L"', addr=" + addrbuf;
                 }
                 
                 dr.indicatorCount = 5; // High severity - code modification
                 if (dr.indicatorCount >= g_cfg.closeThreshold) {
             ...
             // Clear modifications to avoid spam
             g_pMemoryIntegrity->ClearModifications();
         }
         ...
         if (g_pMemoryIntegrity->CheckPageProtection()) {
             auto modifications = g_pMemoryIntegrity->GetModifications();
             for (const auto& mod : modifications) {
                 if (mod.modificationType == "PROTECTION_CHANGED") {
                     DetectionResult dr{};
                     dr.detected = true;
                     dr.pid = GetCurrentProcessId();
                     wchar_t addrbuf[32]; swprintf_s(addrbuf, L"0x%p", mod.address);
                     dr.processName = std::wstring(mod.regionName.begin(), mod.regionName.end());
                     dr.reason = std::wstring(L"Memory protection changed: region='") + dr.processName + 
                                L"', addr=" + addrbuf + 
                                L", expected=0x" + std::to_wstring(mod.expectedProtection) + 
                                L", actual=0x" + std::to_wstring(mod.actualProtection);
                     dr.indicatorCount = 4; // Medium-high severity
                     if (dr.indicatorCount >= g_cfg.closeThreshold) {
                         ProcessDetection(dr, "memory_protection"); 
                         firedLocal = true;
                         break;
                     }
                 }
         ...
         // Check for API hooks (VirtualProtect, VirtualAlloc, etc.)
         if (g_pMemoryIntegrity->DetectApiHooks()) {
             DetectionResult dr{};
             dr.detected = true;
             dr.pid = GetCurrentProcessId();
             dr.processName = L"<api_hooks>";
             dr.reason = L"Memory management API hooks detected (VirtualProtect/VirtualAlloc)";
             dr.indicatorCount = 5; // High severity - API hooking
             if (dr.indicatorCount >= g_cfg.closeThreshold) {
                 ProcessDetection(dr, "api_hook");
                 firedLocal = true;
             }
         }
         
         LogPerf(L"Periodic.MemoryIntegrityMonitor", GetTickCount64() - t0);
         return firedLocal;
     });
 }
...
{
    // Initialize logger first so all subsequent operations can be logged
    Logger::GetInstance().Initialize(g_hModule);
    LOG_INFO("=== Oblivion Anti-Cheat Client Initializing ===");
    
    LoadClientConfig(g_cfg, GetDllDirectory(g_hModule));
    ClampConfig();
    LogIfEnabled(L"[Oblivion] Init thread started\n");
    LOG_INFO("Configuration loaded and clamped");
    ...
    if (g_cfg.enableTelemetry && g_pTelemetryCollector) {
        try {
            ...
            LOG_INFO("ML Feature Extractor initialized successfully");
        } catch (...) {
            g_pMLFeatureExtractor = nullptr;
            LogIfEnabled(L"[Oblivion] ML Feature Extractor failed to initialize\n");
            LOG_ERROR("Failed to initialize ML Feature Extractor");
        }
    }
    ...
    if (g_cfg.enableMLAnomalyDetection && g_pMLFeatureExtractor && g_pTelemetryCollector) {
        try {
            ...
            LOG_INFO("ML Anomaly Detector initialized successfully");
            ...
            LOG_ERROR("Failed to initialize ML Anomaly Detector");
        }
    }
    ...
    if (g_cfg.enableAdaptiveThresholds && g_pTelemetryCollector) {
        try {
            ...
            LOG_INFO("Adaptive Threshold Manager initialized successfully");
        } catch (...) {
            g_pAdaptiveThresholdManager = nullptr;
            LogIfEnabled(L"[Oblivion] Adaptive Threshold Manager failed to initialize\n");
            LOG_ERROR("Failed to initialize Adaptive Threshold Manager");
        }
    }
    ...
    if (g_cfg.enableScanPrioritization) {
        try {
            ...
            g_pScanPrioritizer->RegisterScanner(si.name, si);
                ...
                LOG_INFO("Scan Prioritization Manager initialized with all scanners registered");
            }
            ...
            LOG_ERROR("Failed to initialize Scan Prioritization Manager");
        }
    }
    ...
    if (g_cfg.enableAdaptivePolling) {
        try {
            ...
            LOG_INFO("Adaptive Polling Manager initialized successfully");
        } catch (...) {
            g_pAdaptivePollingManager = nullptr;
            LogIfEnabled(L"[Oblivion] Adaptive Polling Manager failed to initialize\n");
            LOG_ERROR("Failed to initialize Adaptive Polling Manager");
        }
    }

    // ===== PRIORITY 4.4: Initialize Memory Integrity Monitor =====
    if (g_cfg.enableMemoryIntegrity) {
        try {
            ...
            // Auto-register all .text sections if configured
            ...
            LogIfEnabled(L"[Oblivion] Memory Integrity Monitor initialized\n");
            LOG_INFO_FMT("Memory Integrity Monitor initialized - %d regions registered", 
                        g_pMemoryIntegrity->GetRegisteredRegionCount());
        } catch (...) {
            g_pMemoryIntegrity = nullptr;
            LogIfEnabled(L"[Oblivion] Memory Integrity Monitor failed to initialize\n");
            LOG_ERROR("Failed to initialize Memory Integrity Monitor");
        }
    }

        // ===== PRIORITY 4.5: Initialize Anti-Tampering System =====
        if (g_cfg.enableAntiTampering) {
            try {
                ...
                LogIfEnabled(L"[Oblivion] Anti-Tampering System failed to initialize\n");
    ...
    try {
        ...
        if (!g_pNetClient->Initialize(g_cfg.serverIp, g_cfg.serverPort)) {
            ...
            LOG_ERROR_FMT("Network initialization failed - Server: %s:%d", g_cfg.serverIp.c_str(), g_cfg.serverPort);
        } else {
            LOG_INFO_FMT("Network client initialized - Connected to %s:%d", g_cfg.serverIp.c_str(), g_cfg.serverPort);
        }
    } catch (...) {
        LogIfEnabled(L"[Oblivion] Exception during NetworkClient init\n");
        LOG_CRITICAL("Exception caught during NetworkClient initialization");
    ...
    if (g_cfg.enableHmacAuth && !g_cfg.hmacSecret.empty()) {
        ...
        LOG_INFO("HMAC authentication enabled");
    }
    ...
    if (g_cfg.enableHeartbeat) {
        try {
            ...
            LOG_INFO_FMT("Heartbeat started with interval %dms", g_cfg.heartbeatIntervalMs);
        } catch (...) {
    ...
    if (g_cfg.enableKernelBridge) {
        ...
        LOG_INFO("Kernel bridge initialization attempted");
    }
    ...
    try {
        ...
        if (g_pSignatureDB->LoadFromJson(sigPath)) {
            ...
            LogIfEnabled(buf);
            LOG_INFO_FMT("Signature database loaded: %d total, %d enabled", 
                        g_pSignatureDB->GetTotalSignatureCount(),
                        g_pSignatureDB->GetEnabledSignatureCount());
        } else {
            LogIfEnabled(L"[Oblivion] Warning: Failed to load signature database\n");
            LOG_WARNING("Failed to load signature database from " + WToUtf8(sigPath));
            // Continue execution - signature DB is optional
        }
    } catch (...) {
        g_pSignatureDB = nullptr;
        LOG_ERROR("Exception during signature database initialization");
...
static void CleanupGlobals()
{
    ...
    if (g_pTelemetryCollector && g_cfg.telemetryExportOnExit) {
        ...
        LOG_INFO("Telemetry data exported successfully");
    }
    
    // ===== PRIORITY 4.4: Cleanup Memory Integrity Monitor =====
    if (g_pMemoryIntegrity) { 
        g_pMemoryIntegrity->StopMonitoring(); 
        delete g_pMemoryIntegrity; 
        g_pMemoryIntegrity = nullptr; 
        LOG_INFO("Memory Integrity Monitor cleaned up"); 
    }
    
        // ===== PRIORITY 4.5: Cleanup Anti-Tampering System =====
        if (g_pAntiTampering) {
    ...
    if (g_pWatcher) { g_pWatcher->StopBackgroundWatcher(); delete g_pWatcher; g_pWatcher = nullptr; LOG_INFO("ProcessThreadWatcher cleaned up"); }
    if (g_pPeriodic) { g_pPeriodic->Stop(); delete g_pPeriodic; g_pPeriodic = nullptr; LOG_INFO("PeriodicScanner cleaned up"); }
    if (g_pAntiSuspend) { g_pAntiSuspend->Stop(); delete g_pAntiSuspend; g_pAntiSuspend = nullptr; LOG_INFO("AntiSuspend cleaned up"); }
    if (g_pEtw) { g_pEtw->Stop(); delete g_pEtw; g_pEtw = nullptr; LOG_INFO("ETW Heuristics cleaned up"); }
    if (g_pHeartbeat) { g_pHeartbeat->Stop(); delete g_pHeartbeat; g_pHeartbeat = nullptr; LOG_INFO("Heartbeat cleaned up"); }
    if (g_pSigMgr) { g_pSigMgr->Stop(); delete g_pSigMgr; g_pSigMgr = nullptr; LOG_INFO("Signature Manager cleaned up"); }
    if (g_pSignatureDB) { delete g_pSignatureDB; g_pSignatureDB = nullptr; LOG_INFO("Signature Database cleaned up"); }
    // ===== PRIORITY 4: Cleanup Infrastructure Modules =====
    if (g_pScanPrioritizer) { g_pScanPrioritizer->Shutdown(); delete g_pScanPrioritizer; g_pScanPrioritizer = nullptr; LOG_INFO("Scan Prioritizer cleaned up"); }
    if (g_pAdaptiveThresholdManager) { delete g_pAdaptiveThresholdManager; g_pAdaptiveThresholdManager = nullptr; LOG_INFO("Adaptive Threshold Manager cleaned up"); }
    if (g_pAdaptivePollingManager) { delete g_pAdaptivePollingManager; g_pAdaptivePollingManager = nullptr; g_pAdaptivePolling = nullptr; LOG_INFO("Adaptive Polling Manager cleaned up"); }
    if (g_pMLAnomalyDetector) { delete g_pMLAnomalyDetector; g_pMLAnomalyDetector = nullptr; LOG_INFO("ML Anomaly Detector cleaned up"); }
    if (g_pMLFeatureExtractor) { delete g_pMLFeatureExtractor; g_pMLFeatureExtractor = nullptr; LOG_INFO("ML Feature Extractor cleaned up"); }
    if (g_pTelemetryCollector) { g_pTelemetryCollector->Stop(); delete g_pTelemetryCollector; g_pTelemetryCollector = nullptr; g_pTelemetry = nullptr; LOG_INFO("Telemetry Collector cleaned up"); }
    // Cleanup CE detection modules
    if (g_pCEBehavior) { g_pCEBehavior->Stop(); delete g_pCEBehavior; g_pCEBehavior = nullptr; LOG_INFO("CE Behavior Monitor cleaned up"); }
    if (g_pCERegistry) { delete g_pCERegistry; g_pCERegistry = nullptr; LOG_INFO("CE Registry Scanner cleaned up"); }
    if (g_pCEWindow) { delete g_pCEWindow; g_pCEWindow = nullptr; LOG_INFO("CE Window Scanner cleaned up"); }
    if (g_pSpeedHack) { g_pSpeedHack->Stop(); delete g_pSpeedHack; g_pSpeedHack = nullptr; LOG_INFO("Speed Hack Detector cleaned up"); }
    // ===== PRIORITY 2: Cleanup Advanced Detection Modules =====
    if (g_pDeviceScanner) { delete g_pDeviceScanner; g_pDeviceScanner = nullptr; LOG_INFO("Device Scanner cleaned up"); }
    if (g_pNetArtifact) { delete g_pNetArtifact; g_pNetArtifact = nullptr; LOG_INFO("Network Artifact Scanner cleaned up"); }
    // ===== PRIORITY 3: Cleanup Stealth & Evasion Detection Modules =====
    if (g_pPEBDetector) { delete g_pPEBDetector; g_pPEBDetector = nullptr; LOG_INFO("PEB Detector cleaned up"); }
    if (g_pETHREADDetector) { g_pETHREADDetector->Cleanup(); delete g_pETHREADDetector; g_pETHREADDetector = nullptr; LOG_INFO("ETHREAD Detector cleaned up"); }
    if (g_pCallbackScanner) { g_pCallbackScanner->Cleanup(); delete g_pCallbackScanner; g_pCallbackScanner = nullptr; LOG_INFO("Callback Scanner cleaned up"); }
    if (g_pVADDetector) { g_pVADDetector->Cleanup(); delete g_pVADDetector; g_pVADDetector = nullptr; LOG_INFO("VAD Detector cleaned up"); }
    if (g_pHWBPMonitor) { delete g_pHWBPMonitor; g_pHWBPMonitor = nullptr; LOG_INFO("Hardware Breakpoint Monitor cleaned up"); }
    if (g_pMemScanner) { delete g_pMemScanner; g_pMemScanner = nullptr; LOG_INFO("Memory Scanner cleaned up"); }
    if (g_pHeapSpray) { delete g_pHeapSpray; g_pHeapSpray = nullptr; LOG_INFO("Heap Spray Analyzer cleaned up"); }
    if (g_cfg.enableKernelBridge) { KernelBridge_Stop(); LOG_INFO("Kernel Bridge stopped"); }
    if (g_pNetClient) { delete g_pNetClient; g_pNetClient = nullptr; LOG_INFO("Network Client cleaned up"); }
